...
#include <iostream>
#include "PhaseExtractorPolar.h"

using namespace std;

void PhaseExtractorPolar::extract()
{
    int eof = 1;
    //loop through the image. The DataType size is ImageIn->DataSizeIn. The second half is the Phase 
    ...
    while(true)
    {
        ...
        for(int i = 0; i < width*bands; ++i)
        {
            ...
            {
                bufOut[i*sizeIn/2 + j] = bufIn[i*sizeIn + sizeIn/2  + j];
            }
        }
