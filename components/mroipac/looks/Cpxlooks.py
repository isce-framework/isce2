...
from mroipac.looks import cpxlooks

class Cpxlooks(Component):

    def cpxlooks(self):
        dictionary = self.createOptionalArgDictionary()
        if(dictionary):
            cpxlooks.cpxlooks_Py(self.inputImage,self.outputImage,self.width,self.rangeLook,self.azimuthLook,dictionary)
        else:
            cpxlooks.cpxlooks_Py(self.inputImage,self.outputImage,self.width,self.rangeLook,self.azimuthLook)
    ...
    def setPhaseAzimuth(self,var):
        self.pahseAzimuth = float(var)
        return
    
    def setPhaseRange(self,var):
        self.pahseRange = float(var)
        return
    ...
    def __init__(self):
        ...
        self.dictionaryOfVariables = {'RANGE_LOOK' : ['self.rangeLook', 'int','mandatory'], \
                                      ...
                                      'PHASE_RANGE' : ['self.phaseRange', 'float','optional'], \
                                      'PHASE_AZIMUTH' : ['self.phaseAzimuth', 'float','optional'], \
