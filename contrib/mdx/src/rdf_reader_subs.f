...
c     DATA STATEMENTS:
      ...
      data a_version /'<< RDF_READER  Version 32.0    2-March-2006 >>'/
...
c      write(6,'(x,a,a)') 'Reading from: ',a_file(1:max(rdflen(a_file),1))
...
c     PROCESSING STEPS:
      ...
      write(6,*) ' '
c      write(6,'(x,a,a)') 'Reading from: ',a_rdfname(1:max(rdflen(a_rdfname),1))
...
c     PROCESSING STEPS:
      ...
      i_loc = index(a_kkkk,';')
      if (i_loc .gt. 0) then
         read(a_kkkk(i_loc+1:),fmt=*,iostat=i_stat) i_ocr
...
c     DATA STATEMENTS:
      ...
      data a_cnv(1,1) /'nm'/, r_cnv(1,1,1) /1.d-9/,       r_cnv(1,1,2) /0.d0/
      data a_cnv(1,2) /'um'/, r_cnv(1,2,1) /1.d-6/,       r_cnv(1,2,2) /0.d0/
      data a_cnv(1,3) /'mm'/, r_cnv(1,3,1) /1.d-3/,       r_cnv(1,3,2) /0.d0/
      data a_cnv(1,4) /'cm'/, r_cnv(1,4,1) /1.d-2/,       r_cnv(1,4,2) /0.d0/
      data a_cnv(1,5) /'m' /, r_cnv(1,5,1) /1.d0/,        r_cnv(1,5,2) /0.d0/
      data a_cnv(1,6) /'km'/, r_cnv(1,6,1) /1.d+3/,       r_cnv(1,6,2) /0.d0/
      data a_cnv(1,7) /'in'/, r_cnv(1,7,1) /2.54d-2/,     r_cnv(1,7,2) /0.d0/
      data a_cnv(1,8) /'ft'/, r_cnv(1,8,1) /3.048d-1/,    r_cnv(1,8,2) /0.d0/
      data a_cnv(1,9) /'mi'/, r_cnv(1,9,1) /1.609344d3/,  r_cnv(1,9,2) /0.d0/

      data i_cnv(2) /7/                                                         ! area
      data a_cnv(2,1) /'mm*mm'/, r_cnv(2,1,1) /1.d-6/,        r_cnv(2,1,2) /0.d0/
      data a_cnv(2,2) /'cm*cm'/, r_cnv(2,2,1) /1.d-4/,        r_cnv(2,2,2) /0.d0/
      data a_cnv(2,3) /'m*m' /,  r_cnv(2,3,1) /1.d0/,         r_cnv(2,3,2) /0.d0/
      data a_cnv(2,4) /'km*km'/, r_cnv(2,4,1) /1.d+6/,        r_cnv(2,4,2) /0.d0/
      data a_cnv(2,5) /'in*in'/, r_cnv(2,5,1) /6.4516d-4/,    r_cnv(2,5,2) /0.d0/
      data a_cnv(2,6) /'ft*ft'/, r_cnv(2,6,1) /9.290304d-2/,  r_cnv(2,6,2) /0.d0/
      data a_cnv(2,7) /'mi*mi'/, r_cnv(2,7,1) /2.58995511d6/, r_cnv(2,7,2) /0.d0/

      data i_cnv(3) /7/                                                         ! time
      data a_cnv(3,1) /'ns'/, r_cnv(3,1,1) /1.d-9/,  r_cnv(3,1,2) /0.d0/
      data a_cnv(3,2) /'us'/, r_cnv(3,2,1) /1.d-6/,  r_cnv(3,2,2) /0.d0/
      data a_cnv(3,3) /'ms'/, r_cnv(3,3,1) /1.d-3/,  r_cnv(3,3,2) /0.d0/
      data a_cnv(3,4) /'s' /, r_cnv(3,4,1) /1.d0/,   r_cnv(3,4,2) /0.d0/
      data a_cnv(3,5) /'min'/,r_cnv(3,5,1) /6.0d1/,  r_cnv(3,5,2) /0.d0/
      data a_cnv(3,6) /'hr' /,r_cnv(3,6,1) /3.6d3/,  r_cnv(3,6,2) /0.d0/
      data a_cnv(3,7) /'day'/,r_cnv(3,7,1) /8.64d4/, r_cnv(3,7,2) /0.d0/

      data i_cnv(4) /6/                                                         ! velocity
      data a_cnv(4,1) /'cm/s'/,  r_cnv(4,1,1) /1.d-2/,         r_cnv(4,1,2) /0.d0/
      data a_cnv(4,2) /'m/s'/,   r_cnv(4,2,1) /1.d0/,          r_cnv(4,2,2) /0.d0/
      data a_cnv(4,3) /'km/s'/,  r_cnv(4,3,1) /1.d3/,          r_cnv(4,3,2) /0.d0/
      data a_cnv(4,4) /'km/hr'/, r_cnv(4,4,1) /2.77777777777777778d-1/, r_cnv(4,4,2) /0.d0/
      data a_cnv(4,5) /'ft/s'/,  r_cnv(4,5,1) /3.04878d-1/,    r_cnv(4,5,2) /0.d0/
      data a_cnv(4,6) /'mi/hr'/, r_cnv(4,6,1) /4.4704d-1/,     r_cnv(4,6,2) /0.d0/

      data i_cnv(5) /5/                                                         ! power
      data a_cnv(5,1) /'mw'/, r_cnv(5,1,1) /1.d-3/,  r_cnv(5,1,2) /0.d0/
      data a_cnv(5,2) /'w'/,  r_cnv(5,2,1) /1.0d0/,  r_cnv(5,2,2) /0.d0/
      data a_cnv(5,3) /'kw'/, r_cnv(5,3,1) /1.d3/,   r_cnv(5,3,2) /0.d0/
      data a_cnv(5,4) /'dbm'/,r_cnv(5,4,1) /1.d-3/,  r_cnv(5,4,2) /0.d0/
      data a_cnv(5,5) /'dbw'/,r_cnv(5,5,1) /1.0d0/,  r_cnv(5,5,2) /0.d0/

      data i_cnv(6) /4/                                                         ! frequency
      data a_cnv(6,1) /'hz'/, r_cnv(6,1,1) /1.0d0/,  r_cnv(6,1,2) /0.d0/
      data a_cnv(6,2) /'khz'/,r_cnv(6,2,1) /1.0d3/,  r_cnv(6,2,2) /0.d0/
      data a_cnv(6,3) /'mhz'/,r_cnv(6,3,1) /1.0d6/,  r_cnv(6,3,2) /0.d0/
      data a_cnv(6,4) /'ghz'/,r_cnv(6,4,1) /1.0d9/,  r_cnv(6,4,2) /0.d0/

      data i_cnv(7) /3/                                                         ! angle
      data a_cnv(7,1) /'deg'/,r_cnv(7,1,1) /1.0d0/,         r_cnv(7,1,2) /0.d0/
      data a_cnv(7,2) /'rad'/,r_cnv(7,2,1) /57.295779513082320876d0/, r_cnv(7,2,2) /0.d0/
      data a_cnv(7,3) /'arc'/,r_cnv(7,3,1) /0.0002777777777777778d0/, r_cnv(7,3,2) /0.d0/

      data i_cnv(8) /7/                                                         ! data
      data a_cnv(8,1) /'bits'/,  r_cnv(8,1,1) /1.d0/,      r_cnv(8,1,2) /0.d0/
      data a_cnv(8,2) /'kbits'/, r_cnv(8,2,1) /1.d3/,      r_cnv(8,2,2) /0.d0/
      data a_cnv(8,3) /'mbits'/, r_cnv(8,3,1) /1.d6/,      r_cnv(8,3,2) /0.d0/
      data a_cnv(8,4) /'bytes'/, r_cnv(8,4,1) /8.d0/,      r_cnv(8,4,2) /0.d0/
      data a_cnv(8,5) /'kbytes'/,r_cnv(8,5,1) /8320.d0/,   r_cnv(8,5,2) /0.d0/
      data a_cnv(8,6) /'mbytes'/,r_cnv(8,6,1) /8388608.d0/,r_cnv(8,6,2) /0.d0/
      data a_cnv(8,7) /'words'/, r_cnv(8,7,1) /32.d0/,     r_cnv(8,7,2) /0.d0/

      data i_cnv(9) /7/                                                         ! data rate
      data a_cnv(9,1) /'bits/s'/,  r_cnv(9,1,1) /1.d0/,      r_cnv(9,1,2) /0.d0/
      data a_cnv(9,2) /'kbits/s'/, r_cnv(9,2,1) /1.d3/,      r_cnv(9,2,2) /0.d0/
      data a_cnv(9,3) /'mbits/s'/, r_cnv(9,3,1) /1.d6/,      r_cnv(9,3,2) /0.d0/
      data a_cnv(9,4) /'bytes/s'/, r_cnv(9,4,1) /8.d0/,      r_cnv(9,4,2) /0.d0/
      data a_cnv(9,5) /'kbytes/s'/,r_cnv(9,5,1) /8320.d0/,   r_cnv(9,5,2) /0.d0/
      data a_cnv(9,6) /'mbytes/s'/,r_cnv(9,6,1) /8388608.d0/,r_cnv(9,6,2) /0.d0/
      data a_cnv(9,7) /'baud'/,    r_cnv(9,7,1) /1.d0/,      r_cnv(9,7,2) /0.d0/

      data i_cnv(10) /3/                                                                ! temperature
      data a_cnv(10,1) /'deg c'/,r_cnv(10,1,1) /1.0d0/,      r_cnv(10,1,2) /0.d0/
      data a_cnv(10,2) /'deg k'/,r_cnv(10,2,1) /1.0d0/,      r_cnv(10,2,2) /273.0d0/
      data a_cnv(10,3) /'deg f'/,r_cnv(10,3,1) /0.555556d0/, r_cnv(10,3,2) /-32.d0/

      data i_cnv(11) /2/                                                                ! ratio
      data a_cnv(11,1) /'-'/, r_cnv(11,1,1) /1.0d0/,  r_cnv(11,1,2) /0.0d0/
      data a_cnv(11,2) /'db'/,r_cnv(11,2,1) /1.0d0/,  r_cnv(11,2,2) /0.0d0/

      data i_cnv(12) /2/                                                                ! fringe rate
      data a_cnv(12,1) /'deg/m'/,r_cnv(12,1,1) /1.0d0/        ,  r_cnv(12,1,2) /0.0d0/
      data a_cnv(12,2) /'rad/m'/,r_cnv(12,2,1) /57.295779513082320876d0/,  r_cnv(12,2,2) /0.0d0/
...
c     PROCESSING STEPS:
     ...
     &                 a_unit(1:max(min(rdflen(a_unit),150),2))//'  '//
     &                 a_ounit(1:max(min(rdflen(a_ounit),150),2))
         call rdf_error(a_errtmp)
         call rdf_trace(' ')
         return
      endif
...
c     PROCESSING STEPS:
      ...
      if (a_intfmt .eq. '*') then
        ...
        write(a_fmt,'(a,i7.7,a,a)') '(',i_num,a_intfmt(1:max(rdflen(a_intfmt),1)),')'
...
c     PROCESSING STEPS:
      ...
      if (a_realfmt .eq. '*') then
        ...
        write(a_fmt,'(a,i7.7,a,a)') '(',i_num,a_realfmt(1:max(rdflen(a_realfmt),1)),')'
...
c     PROCESSING STEPS:
      ...
      if (a_dblefmt .eq. '*') then
        ...
        write(a_fmt,'(a,i7.7,a,a)') '(',i_num,'('//a_dblefmt(1:max(rdflen(a_dblefmt),1)),',x))'
        write(unit=a_outval,fmt=a_fmt) (r_data(i),i=1,i_num)
      endif
      rdfdble=a_outval
      call rdf_trace(' ')
...
c      write(6,*) 'here =',a_string(1:60)
      ...
      i_pos=len(a_string)
      do while(i_pos .gt. 0)
        a_val=a_string(i_pos:i_pos) 
        if (a_val .eq. ' ' .or. ichar(a_val) .eq. 0) then
          i_pos=i_pos-1
        else
          i_len=i_pos
          i_pos=0
...
c     PROCESSING STEPS:
      ...
      do i=1,i_fields
         ...
         if (i_pos .gt. 0) then
            if (i_pos .gt. 1) then
               read(a_fields(i)(1:i_pos-1),fmt=*,iostat=i_stat) i_order(i)
               if (i_stat .ne. 0) then
         ...
         if (i_pos .gt. 0) then
            if (i_pos .gt. 1) then
               read(a_fields(i)(1:i_pos-1),fmt=*,iostat=i_stat) i_strt(i)
               if (i_stat .ne. 0) then
         ...
         read(unit=a_fields(i)(1:i_pos),fmt=*,iostat=i_stat) i_stop(i)
         if (i_stat .ne. 0) then
